            ...
            <div
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-body transition-all ${
                i === 0
                  ...
                  : "bg-navy-800 text-cream-300/50 border border-gold-500/10"
              }`}
            >
              {f}
            </div>
          ...
          {DEMO_REPORTS.map((report, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Failed error */}
                {report.status === "FAILED" && "error" in report && (
                  <p className="text-red-400/60 text-xs font-body mb-3 line-clamp-1">
                    {(report as typeof report & { error?: string }).error}
                  </p>
                )}
